                      ...
                      <tbody>
                        ...
                        <tr>
                          ...
                          <td>
                            ...
                            <form action="{{ route('partner.foreign_assets.destroy', $foreignAsset->id) }}" method="POST" style="display:inline;">
                              @csrf
                              @method('DELETE')
...
<script>
  $(document).ready(function() {
    $('#saveForeignAsset').on('click', function() {
      $.ajax({
        type: 'POST',
        url: "{{ route('partner.foreign_assets.store') }}",
    ...
    $('#updateForeignAsset').on('click', function() {
      ...
      $.ajax({
        type: 'POST',
        url: '/partner/foreign_assets/update/' + id,
  ...
  $('#saveCustomAssetType').on('click', function () {
    ...
    if (customAssetType) {
      $.ajax({
        type: 'POST',
        url: "{{ route('partner.foreign_assets.save_custom_type') }}",
        data: {
          _token: "{{ csrf_token() }}",
