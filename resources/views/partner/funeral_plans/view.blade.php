                      ...
                      <tbody>
                        @foreach($funeralPlans as $plan)
                            <tr>
                                ...
                                <form action="{{ route('partner.funeral_plans.destroy', $plan->id) }}" method="POST"
...
<script>
  $(document).ready(function () {
    ...
    $('#addFuneralPlanForm').on('submit', function (e) {
      ...
      $.ajax({
        url: "{{ route('partner.funeral_plans.store') }}",
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
    ...
    $('#editFuneralPlanForm').on('submit', function (e) {
      ...
      $.ajax({
        url: "{{ url('partner/funeral_plans/update') }}" + '/' + funeralPlanId, // Include the ID in the URL
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
  ...
  $('#saveCustomFuneralPlan').on('click', function () {
    ...
    if (customFuneralPlan) {
      $.ajax({
        type: 'POST',
        url: "{{ route('partner.funeral_plans.save_custom_type') }}",
        data: {
          _token: "{{ csrf_token() }}",
