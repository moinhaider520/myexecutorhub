...
@section('content')
  <div class="page-body">
    ...
    @endif
          ...
          <form action="{{ route('partner.guidance.destroy', $document->id) }}" method="POST"
  ...
  <script>
    ...
    $('#addDocumentForm').on('submit', function (e) {
      ...
      $.ajax({
      url: "{{ route('partner.guidance.store') }}",
      type: "POST",
      data: formData,
      contentType: false,
      processData: false,
    ...
    $('.edit-button').on('click', function () {
      ...
      $.get(`/partner/guidance/${id}/media`, function (media) {
      if (media.length > 0) {
    ...
    $(document).on('click', '.delete-media', function () {
      ...
      $.ajax({
      url: `/partner/guidance/media/${mediaId}`,
      type: 'DELETE',
    ...
    if ($(this).val() === 'Others') {
      ...
      $.ajax({
        url: "{{ url('partner/guidance/update') }}" + '/' + documentId, // Include the ID in the URL
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
