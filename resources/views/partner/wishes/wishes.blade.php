                    ...
                    <table class="display dataTable no-footer" id="basic-1" role="grid" aria-describedby="basic-1_info">
                      ...
                      <tr>
                        ...
                        <td>
                          ...
                          <form action="{{ route('partner.wishes.destroy', $document->id) }}" method="POST"
...
<script>
  $(document).ready(function() {
    ...
    $('#addDocumentForm').on('submit', function(e) {
      ...
      $.ajax({
        url: "{{ route('partner.wishes.store') }}",
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
    ...
    $('.edit-button').on('click', function() {
      ...
      $.get(`/partner/wishes/${id}/media`, function(media) {
        if (media.length > 0) {
    ...
    $(document).on('click', '.delete-media', function() {
      ...
      $.ajax({
        url: `/partner/wishes/media/${mediaId}`,
        type: 'DELETE',
        data: {
          _token: '{{ csrf_token() }}'
        },
  ...
  $('#editDocumentForm').on('submit', function(e) {
    ...
    $.ajax({
      url: "{{ url('partner/wishes/update') }}" + '/' + documentId, // Include the ID in the URL
      type: "POST",
      data: formData,
      contentType: false,
      processData: false,
