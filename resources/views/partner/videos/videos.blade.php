                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td>
                          ...
                          <form action="{{ route('partner.videos.destroy', $video->id) }}" method="POST" style="display:inline;">
                            @csrf
                            @method('DELETE')
...
<div class="modal fade" id="addVideoModal" tabindex="-1" role="dialog">
  <div class="modal-dialog" role="document">
    <form id="addVideoForm" action="{{ route('partner.videos.store') }}" method="POST" enctype="multipart/form-data">
      @csrf
...
$(document).ready(function () {
  ...
  $('.edit-video-button').on('click', function () {
    ...
    $('#editVideoForm').attr('action', '/partner/videos/update/' + $(this).data('id'));
  });
  ...
  $('#addVideoForm').on('submit', function (e) {
    ...
    $.ajax({
      url: "{{ route('partner.videos.store') }}",
      type: "POST",
      data: formData,
      contentType: false,
      processData: false,
