                    ...
                    <table class="display dataTable no-footer" id="basic-1" role="grid" aria-describedby="basic-1_info">
                      ...
                      <tr>
                        ...
                        <td>
                          ...
                          <form action="{{ route('partner.life_remembered_videos.destroy', $video->id) }}" method="POST"
...
<script>
  $(document).ready(function() {
    ...
    $('#addVideoForm').on('submit', function(e) {
      ...
      $.ajax({
        url: "{{ route('partner.life_remembered_videos.store') }}",
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
    ...
    $('.edit-button').on('click', function() {
      ...
      $.get(`/partner/life_remembered_videos/${id}/media`, function(media) {
        if (media.length > 0) {
    ...
    $(document).on('click', '.delete-video', function() {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          $.ajax({
            url: `/partner/life_remembered_videos/media/${videoId}`,
            type: 'DELETE',
            data: {
              _token: '{{ csrf_token() }}'
            },
    ...
    $('#editVideoForm').on('submit', function(e) {
      ...
      $.ajax({
        url: "{{ url('partner/life_remembered_videos/update') }}" + '/' + videoId,
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
