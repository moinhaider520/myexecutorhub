                      ...
                      <tbody>
                        @foreach($funeralPlans as $plan)
                            <tr>
                                ...
                                <form action="{{ route('customer.funeral_plans.destroy', $plan->id) }}" method="POST"
...
<script>
  $(document).ready(function () {
    ...
    $('#addFuneralPlanForm').on('submit', function (e) {
      ...
      $.ajax({
        url: "{{ route('customer.funeral_plans.store') }}",
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
    ...
    $('#editFuneralPlanForm').on('submit', function (e) {
      ...
      $.ajax({
        url: "{{ url('customer/funeral_plans/update') }}" + '/' + funeralPlanId, // Include the ID in the URL
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
  ...
  $('#saveCustomFuneralPlan').on('click', function () {
    ...
    if (customFuneralPlan) {
      $.ajax({
        type: 'POST',
        url: "{{ route('customer.funeral_plans.save_custom_type') }}",
        data: {
          _token: "{{ csrf_token() }}",
