                          ...
                          @foreach($documents as $document)
                            <tr>
                              ...
                              <td>
                                ...
                                <form action="{{ route('executor.documents.destroy', $document->id) }}" method="POST"
        ...
        <div class="modal-body">
          <form id="editDocumentForm" enctype="multipart/form-data">
            ...
            <div class="form-group mb-4">
              ...
              <input type="file" class="form-control" name="files[]" id="editFile" multiple>
  ...
  <script>
    $(document).ready(function () {
      ...
      $('#addDocumentForm').on('submit', function (e) {
        ...
        $.ajax({
          url: "{{ route('executor.documents.store') }}",
          type: "POST",
          data: formData,
          contentType: false,
          processData: false,
      ...
      $('#editDocumentForm').on('submit', function (e) {
        ...
        $.ajax({
          url: "{{ url('executor/documents/update') }}" + '/' + documentId, // Include the ID in the URL
          type: "POST",
          data: formData,
          contentType: false,
          processData: false,
    ...
    $('#saveCustomDocumentType').on('click', function () {
      ...
      if (customDocumentType) {
        $.ajax({
          type: 'POST',
          url: "{{ route('executor.documents.save_custom_type') }}",
          data: {
            _token: "{{ csrf_token() }}",
