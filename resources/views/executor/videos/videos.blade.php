                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td>
                          ...
                          <form action="{{ route('executor.videos.destroy', $video->id) }}" method="POST" style="display:inline;">
                            @csrf
                            @method('DELETE')
...
<div class="modal fade" id="addVideoModal" tabindex="-1" role="dialog">
  <div class="modal-dialog" role="document">
    <form id="addVideoForm" action="{{ route('executor.videos.store') }}" method="POST" enctype="multipart/form-data">
      @csrf
...
$(document).ready(function () {
  ...
  $('.edit-video-button').on('click', function () {
    ...
    $('#editVideoForm').attr('action', '/executor/videos/update/' + $(this).data('id'));
  });
  ...
  $('#addVideoForm').on('submit', function (e) {
    ...
    $.ajax({
      url: "{{ route('executor.videos.store') }}",
      type: "POST",
      data: formData,
      contentType: false,
      processData: false,
