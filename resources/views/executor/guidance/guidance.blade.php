...
@section('content')
  <div class="page-body">
    ...
    @endif
          ...
          <form action="{{ route('executor.guidance.destroy', $document->id) }}" method="POST"
  ...
  <script>
    ...
    $('#addDocumentForm').on('submit', function (e) {
      ...
      $.ajax({
      url: "{{ route('executor.guidance.store') }}",
      type: "POST",
      data: formData,
      contentType: false,
      processData: false,
    ...
    $('.edit-button').on('click', function () {
      ...
      $.get(`/executor/guidance/${id}/media`, function (media) {
      if (media.length > 0) {
    ...
    $(document).on('click', '.delete-media', function () {
      ...
      $.ajax({
      url: `/executor/guidance/media/${mediaId}`,
      type: 'DELETE',
    ...
    if ($(this).val() === 'Others') {
      ...
      $.ajax({
        url: "{{ url('executor/guidance/update') }}" + '/' + documentId, // Include the ID in the URL
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
