                      ...
                      <tbody>
                        ...
                        <tr>
                          ...
                          <td style="display: flex;">
                            ...
                            <form action="{{ route('executor.insurance_policies.destroy', $insurancePolicy->id) }}" method="POST" style="display:inline;">
                              @csrf
                              @method('DELETE')
...
<script>
  ...
  $('#saveInsurancePolicy').on('click', function() {
    ...
    $.ajax({
      type: 'POST',
      url: '{{ route("executor.insurance_policies.store") }}',
      data: formData,
  ...
  $('#updateInsurancePolicy').on('click', function() {
    ...
    $.ajax({
      type: 'POST',
      url: '{{ route("executor.insurance_policies.update", "") }}/' + id,
      data: formData,
      success: function(response) {
        // Reset form fields if needed
  ...
  $('#insuranceType').change(function () {
    ...
    $('#saveCustomInsuranceType').on('click', function () {
      ...
      if (customInsuranceType) {
        $.ajax({
          type: 'POST',
          url: "{{ route('executor.insurance_policies.save_custom_type') }}",
          data: {
            _token: "{{ csrf_token() }}",
