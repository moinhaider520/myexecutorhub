                      ...
                      <tbody>
                        ...
                        <tr>
                          ...
                          <td>
                            ...
                            <form action="{{ route('executor.other_type_of_assets.destroy', $otherAsset->id) }}" method="POST" style="display:inline;">
                              @csrf
                              @method('DELETE')
...
<script>
  $(document).ready(function() {
    $('#saveOtherAsset').on('click', function() {
      $.ajax({
        type: 'POST',
        url: "{{ route('executor.other_type_of_assets.store') }}",
    ...
    $('#updateOtherAsset').on('click', function() {
      ...
      $.ajax({
        type: 'POST',
        url: '/executor/other_type_of_assets/update/' + id,
  ...
  $('#assetType').change(function () {
    ...
    $('#saveCustomAssetType').on('click', function () {
      ...
      if (customAssetType) {
        $.ajax({
          type: 'POST',
          url: "{{ route('executor.other_type_of_assets.save_custom_type') }}",
          data: {
            _token: "{{ csrf_token() }}",
