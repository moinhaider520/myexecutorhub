                      ...
                      <tbody>
                        ...
                        <tr>
                          ...
                          <td>
                            ...
                            <form action="{{ route('executor.personal_chattels.destroy', $chattel->id) }}" method="POST" style="display:inline;">
                              @csrf
                              @method('DELETE')
...
<script>
  $(document).ready(function() {
    $('#saveChattel').on('click', function() {
      $.ajax({
        type: 'POST',
        url: "{{ route('executor.personal_chattels.store') }}",
    ...
    $('#updateChattel').on('click', function() {
      ...
      $.ajax({
        type: 'POST',
        url: "{{ url('executor/personal_chattels/update') }}/" + id,
  ...
  $('#saveCustomChattelType').on('click', function() {
    ...
    if (customChattelType) {
      $.ajax({
        type: 'POST',
        url: "{{ route('executor.personal_chattels.save_custom_type') }}",
        data: {
          _token: "{{ csrf_token() }}",
