                      ...
                      <tbody>
                        ...
                        <tr>
                          ...
                          <td style="display:flex;">
                            ...
                            <form action="{{ route('executor.business_interests.destroy', $businessInterest->id) }}" method="POST" style="display:inline;">
                              @csrf
                              @method('DELETE')
...
<script>
  ...
  $('#saveBusinessInterest').on('click', function () {
    ...
    $.ajax({
      type: 'POST',
      url: '{{ route("executor.business_interests.store") }}',
      data: formData,
  ...
  $('#updateBusinessInterest').on('click', function () {
    ...
    $.ajax({
      type: 'POST',
      url: '/executor/business_interests/update/' + id,
      data: formData,
      success: function (response) {
        // Reset form fields if needed
  ...
  $('#businessType').change(function () {
    ...
    $('#saveCustomBusinessType').on('click', function () {
      ...
      if (customBusinessType) {
        $.ajax({
          type: 'POST',
          url: "{{ route('executor.business_interests.save_custom_type') }}",
          data: {
            _token: "{{ csrf_token() }}",
