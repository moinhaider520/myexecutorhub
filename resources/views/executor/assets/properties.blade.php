...
@section('content')
      ...
      <div class="row">
        ...
        <div class="card">
          ...
          <button type="button" class="btn btn-warning btn-sm edit-button" data-toggle="modal"
            data-target="#editPropertyModal" data-id="{{ $property->id }}"
            data-property_type="{{ $property->property_type }}"
            data-address="{{ $property->address }}" data-owner_names="{{ $property->owner_names }}"
            data-how_owned="{{ $property->how_owned }}"
            data-value="{{ $property->value }}">Edit</button>
          <form action="{{ route('executor.properties.destroy', $property->id) }}" method="POST"
  ...
  <div class="modal fade" id="addPropertyModal" tabindex="-1" role="dialog" aria-labelledby="addPropertyModalLabel"
    ...
    <div class="modal-content">
      ...
      @endforeach
        ...
        <input type="text" class="form-control" name="owner_names" id="ownerName" placeholder="Owner Name(s)"
          required>
  ...
  <!-- EDIT PROPERTY MODAL -->
  <div class="modal fade" id="editPropertyModal" tabindex="-1" role="dialog" aria-labelledby="editPropertyModalLabel"
    aria-hidden="true">
    <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
      ...
      @endforeach
        ...
        <input type="text" class="form-control" name="custom_property_type" id="edit_custom_property_type"
          placeholder="Enter Custom Property Type">
        ...
        <input type="text" class="form-control" name="owner_names" id="editOwnerName" placeholder="Owner Name(s)"
          required>
        ...
        <input type="number" class="form-control" name="value" id="editValuation" placeholder="Enter Value"
          required>
  ...
  <script>
    ...
    $('#saveProperty').on('click', function () {
      $.ajax({
      type: 'POST',
      url: "{{ route('executor.properties.store') }}",
    ...
    $('#updateProperty').on('click', function () {
      ...
      $.ajax({
      type: 'PUT',
      url: "{{ url('executor/properties/update') }}/" + id,
    ...
    $('#saveCustomPropertyType').on('click', function () {
      ...
      $.ajax({
        type: 'POST',
        url: "{{ route('executor.properties.save_custom_type') }}",
        data: {
        _token: "{{ csrf_token() }}",
