                      ...
                      <tbody>
                        ...
                        <tr>
                          ...
                          <td style="display: flex;">
                            ...
                            <form action="{{ route('executor.digital_assets.destroy', $digitalAsset->id) }}" method="POST" style="display:inline;">
                              @csrf
                              @method('DELETE')
...
<script>
  ...
  $('#saveDigitalAsset').on('click', function () {
    ...
    $.ajax({
      type: 'POST',
      url: '{{ route("executor.digital_assets.store") }}',
      data: formData,
  ...
  $('#updateDigitalAsset').on('click', function () {
    ...
    $.ajax({
      type: 'POST',
      url: '/executor/digital_assets/update/' + id,
      data: formData,
      success: function (response) {
        // Reset form fields
  ...
  $('#assetType').change(function () {
    ...
    $('#saveCustomAssetType').on('click', function () {
      ...
      if (customAssetType) {
        $.ajax({
          type: 'POST',
          url: "{{ route('executor.digital_assets.save_custom_type') }}",
          data: {
            _token: "{{ csrf_token() }}",
