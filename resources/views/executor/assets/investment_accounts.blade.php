                      ...
                      <tbody>
                        ...
                        <tr>
                          ...
                          <td>
                            ...
                            <form action="{{ route('executor.investment_accounts.destroy', $investmentAccount->id) }}" method="POST" style="display:inline;">
                              @csrf
                              @method('DELETE')
...
<script>
  $(document).ready(function() {
    $('#saveInvestmentAccount').on('click', function() {
      $.ajax({
        type: 'POST',
        url: "{{ route('executor.investment_accounts.store') }}",
    ...
    $('#updateInvestmentAccount').on('click', function() {
      ...
      $.ajax({
        type: 'POST',
        url: '/executor/investment_accounts/update/' + id,
    ...
    $('#saveCustomInvestmentType').on('click', function () {
      ...
      if (customInvestmentType) {
        $.ajax({
          type: 'POST',
          url: "{{ route('executor.investment_accounts.save_custom_type') }}",
          data: {
            _token: "{{ csrf_token() }}",
