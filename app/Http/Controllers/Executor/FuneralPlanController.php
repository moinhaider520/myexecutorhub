...
class FuneralPlanController extends Controller
{
    ...
    public function view()
    {
        ...
        $userIds = collect([$authUser->id, $contextUser->id])
            ->unique()
            ->values();
        $funeralPlans = FuneralPlan::whereIn('created_by', $userIds)->get();
        $funeralPlanTypes = CustomDropDown::where('category', 'funeral_plan')->get();
        return view('executor.funeral_plans.view', compact('funeralPlans', 'funeralPlanTypes'));
    }
    ...
    public function destroy(string $id)
    {
        try {
            ...
            DB::commit();
            return redirect()->route('executor.funeral_plans.view')->with('success', 'Funeral plan deleted successfully.');
        } catch (\Exception $e) {
            DB::rollback();
