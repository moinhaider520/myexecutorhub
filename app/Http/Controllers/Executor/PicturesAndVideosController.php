...
class PicturesAndVideosController extends Controller
{
    ...
    public function view()
    {
        ...
        $userIds = collect([$authUser->id, $contextUser->id])
            ->unique()
            ->values();
        $pictures_and_videos = PicturesAndVideos::whereIn('created_by', $userIds)->get();
    ...
    public function destroy($id)
    {
        try {
            ...
            $document->delete();
            DB::commit();
            return redirect()->route('executor.pictures_and_videos.view')->with('success', 'File deleted successfully.');
        } catch (\Exception $e) {
            DB::rollback();
