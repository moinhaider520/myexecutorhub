...
class PropertyController extends Controller
{
    public function view()
    {
        ...
        $userIds = collect([$authUser->id, $contextUser->id])
            ->unique()
            ->values();
        $propertyTypes = PropertyType::whereIn('created_by', $userIds)->get();
        $properties = Property::whereIn('created_by', $userIds)->get();
    ...
    public function destroy($id)
    {
        try {
            ...
            DB::commit();
            return redirect()->route('executor.properties.view')->with('success', 'Property deleted successfully.');
        } catch (\Exception $e) {
            DB::rollback();
